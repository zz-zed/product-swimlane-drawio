...
- **Check explicitly declared process relationships:** read [semantic context v1](references/semantic-context.md). This opt-in, read-only check is for a managed v3 diagram and a supplied context file; it checks declared patterns and group relationships without inferring process facts or repairing the diagram.
- **Record declared source assertions and field bindings:** read [provenance context v1](references/provenance.md). Use its explicit context bundle workflow only when the requester needs source/fact/binding records preserved through a build, patch, comparison, or eligible migration; it never reads a source reference or authenticates a confirmation.
- **Prepare or record visual evidence:** read [visual review evidence v1](references/visual-review.md). Use it only for an immutable snapshot and externally supplied preview/reviewer evidence; it does not render, inspect an image, or repair the diagram.
- **Construct and assess an authorized visual-repair candidate:** read [authorized visual-repair candidates](references/visual-repair.md). Use it only with explicit edge-level authorization, immutable prepared/record packages, fresh candidate evidence, and an assessment receipt; it never replaces the original diagram in place.
